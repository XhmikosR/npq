...
const { isInteractiveTerminal } = require('./cliSupportHandler.js')
...
function getDisplayLength(str) {
  ...
  return str.replaceAll(/\u001B\[[\d;]*m/g, '').length
}
...
function reportResults(marshallResults, { plain = false } = {}) {
  ...
  let countErrors = 0
  let countWarnings = 0
  const countPackages = results.length
  ...
  for (const [index, result] of results.entries()) {
    ...
    if (!isPackageMalicious && result.warnings && result.warnings.length > 0) {
      for (const warningArray of result.warnings) {
