...
const Marshalls = require('./marshalls.js')
const PackageRepoUtils = require('./helpers/packageRepoUtils.js')
...
class Marshall {
  ...
  createPackageVersionMaps(packages) {
    const packageVersionMapping = packages.reduce((prev, curr) => {
      ...
      prev.push({
        packageName: curr.slice(0, Math.max(0, versionPosition)),
        packageVersion: curr.slice(versionPosition + 1) || 'latest',
        packageString: curr
      })
