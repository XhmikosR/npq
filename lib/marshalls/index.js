...
const Marshalls = {
  ...
  async buildMarshallTasks(marshalls, options) {
    if (!marshalls || !Array.isArray(marshalls) || marshalls.length === 0) {
      throw new Error('unable to collect marshalls, or no marshalls found')
    }
   ...
   */
  async tasks(options, progressManager) {
  ...
  warmUpPackagesCache(options) {
    const fetchPackagesInfoPromises = []
    for (const packageMeta of options.pkgs) {
