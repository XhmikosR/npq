...
class Marshall extends BaseMarshall {
  ...
  async validate(pkg) {
    ...
    if (
      (!firstVersionForUser || firstVersionForUser.version === packageVersion) && // Only throw the error if also the `packageVersion` was published less than 21 days ago:
      versionPublishedDateString
    ) {
      const dateDiffInMs = Date.now() - new Date(versionPublishedDateString)
